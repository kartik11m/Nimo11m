...
async function sendSubscriptionNotification({ to, email }) {
  const transport = getTransporter()
  ...
  const html = `
    <div style="font-family: Arial, sans-serif; background: #050508; color: #f5f1e8; padding: 24px; border-radius: 16px; max-width: 640px; margin: 0 auto;">
      <div style="background: linear-gradient(90deg, #ff6b35, #8b31e8); padding: 2px; border-radius: 16px;">
        <div style="background: #0f0f14; border-radius: 14px; padding: 28px;">
          <h2 style="margin: 0 0 10px; color: #ff6b35; font-size: 24px;">New subscriber joined</h2>
          <p style="font-size: 15px; line-height: 1.7; color: #e8e4d8;">
            Congratulations! A new subscriber has signed up for updates from Nimo Labs.
          </p>
          <div style="margin: 20px 0; padding: 14px 18px; border: 1px solid rgba(255,107,53,0.25); background: rgba(255,107,53,0.08); border-radius: 10px;">
            <p style="margin: 0; font-size: 14px; color: #ffb08a;"><strong>Email:</strong> ${email}</p>
          </div>
          <p style="margin: 0; font-size: 13px; color: #9ca3af;">Warm regards,<br/>The Nimo Labs Automation</p>
        </div>
      </div>
    </div>
  `
  ...
  await transport.sendMail({
    ...
    to,
    replyTo: email,
    subject: 'New newsletter subscriber signed up',
    text: `A new subscriber has signed up for Nimo Labs updates.\n\nEmail: ${email}`,
    html,
  })
...
async function sendSubscriptionConfirmationEmail({ to, email }) {
  const transport = getTransporter()
  ...
  const html = `
    <div style="font-family: Arial, sans-serif; background: #050508; color: #f5f1e8; padding: 24px; border-radius: 16px; max-width: 640px; margin: 0 auto;">
      <div style="background: linear-gradient(90deg, #ff6b35, #8b31e8); padding: 2px; border-radius: 16px;">
        <div style="background: #0f0f14; border-radius: 14px; padding: 28px;">
          <h2 style="margin: 0 0 10px; color: #ff6b35; font-size: 24px;">You’re subscribed!</h2>
          <p style="font-size: 15px; line-height: 1.7; color: #e8e4d8;">
            Thanks for subscribing to Nimo Labs updates. You’ll now receive camp news, project showcases, and early-bird offers straight to your inbox.
          </p>
          <div style="margin: 20px 0; padding: 14px 18px; border: 1px solid rgba(255,107,53,0.25); background: rgba(255,107,53,0.08); border-radius: 10px;">
            <p style="margin: 0; font-size: 14px; color: #ffb08a;">Subscribed email: ${email}</p>
          </div>
          <p style="margin: 0; font-size: 13px; color: #9ca3af;">Warm regards,<br/>The Nimo Labs Team</p>
        </div>
      </div>
    </div>
  `
  ...
  await transport.sendMail({
    ...
    to,
    replyTo: process.env.SMTP_FROM || process.env.SMTP_USER,
    subject: 'You’re subscribed to Nimo Labs updates',
    text: `Hi there,\n\nThanks for subscribing to Nimo Labs updates. You'll now receive camp news, project showcases, and early-bird offers straight to your inbox.\n\nWarm regards,\nThe RoboLearn Team`,
    html,
  })
...
module.exports = { sendEnquiryEmail, sendThankYouEmail, sendSubscriptionNotification, sendSubscriptionConfirmationEmail }
